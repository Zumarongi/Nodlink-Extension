...
if __name__ == "__main__":
    ...
    for node in G:
        if G.nodes[node]['type'] == NODE_TYPE.PROCESS:#进程节点
            if G.nodes[node]['cmd'] == '' and G.nodes[node]['label'] == '':#label:md5值，表示节点id
                continue
            ...
            for i in G.successors(node):
                ...
                elif G.nodes[i]['type'] == NODE_TYPE.PROCESS and G.nodes[node]['cmd']!='':
                    data.write(G.nodes[i]['cmd']+' cmd_'+'\n')
