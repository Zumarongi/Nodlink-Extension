...
from config import *
from tools import *
def get_size(obj, seen=None):#递归计算对象占用内存大小
...
class ProvGraph(object):
    def __init__(self,dataset):
        self.lock = threading.Lock()# 初始化一个线程锁，用于线程同步
        self.thread_lock = threading.Lock()
        self.G = nx.DiGraph() #初始化一个有向图G，用于存储事件和它们之间的关系。
        self.TmpG = nx.DiGraph()# 初始化一个临时有向图TmpG，用于临时存储事件关系。
        self.w2v = FastText.load('../' + dataset + '/filepath-embedding.model')
        self.c2v = FastText.load('../' + dataset + '/cmdline-embedding.model')
        self.tfidf = json.load(open('../' + dataset + '/tfidf.json',encoding='utf-8'))
        self.taylor_map = dict()# 初始化一个字典，用于存储节点的映射
        self.mean_tfidf = np.mean(list(self.tfidf.values()))
        self.AS = AnomalyScore(dataset)#初始化图的异常指数
        self.graph_cache = []# 初始化图缓存列表，用于存储临时图快照
        self.node_set = set()#已访问的节点
        self.filtered = set()
        self.hit = set()# 命中了的节点过程
        if dataset == 'hw20':
    ...
    #向临时图中添加边
    def graph_add_node_mgr(self, row, key, event_type):# 参数 row 是日志记录，key 表示事件类型，event_type 用于存储边的类型信息。
        self.lock.acquire()
        node_attr = {}
    ...
    def GetNodeCmd(self,node):
        ...
        return x
    def GetEmbedding(self, corpus, model):# 参数 corpus 是一个文本列表，model 是一个预训练的词嵌入模型，获取文本列表的平均嵌入
    ...
    def update(self, anomaly_cutoff, topK = 20):# update 方法定义，用于更新图并计算节点的异常分数，保留topK个异常系数最高的点
        self.lock.acquire()
#        self.G = nx.compose(self.G,self.TmpG)
        self.G = self.TmpG.copy()#将临时图赋值给当前图
        self.TmpG.clear()
        self.lock.release()
    ...
    def caculate_anomaly_score(self,pnode,anomaly_cutoff):
        ...
        for node in pnode:
            ...
            for nei in neibor:
                ...
                #    need_to_caculate[node].append((nei,self.GetNodeName(nei)))#待计算列表：进程+与之相关的非进程节点
        ...
        if len(need_to_caculate) == 0:
            return -1
        node_feature = defaultdict(list)#节点的特征向量
        f = True
        for node in need_to_caculate.keys():
            flag = False
            cmdline = self.GetNodeCmd(node)#获取节点cmdline
            split_path = sanitize_string(cmdline)
            if len(split_path) == 0:
                continue
            new_name = '/'.join(split_path)
            self.nodes[node]['newname'] = new_name#设置newname
            tmp = []
