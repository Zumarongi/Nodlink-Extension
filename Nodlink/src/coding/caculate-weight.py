...
def extract_process_feature(file_path,w2v,c2v):
    process_map = {}
    f = open(file_path,'r')
    print('start graph')
    ...
    while True:
        ...
        #added in mytry
        if filepath.endswith(' cmd_'):
            if not filepath:
                continue
            split_path=sanitize_string(filepath)[0:-1]
...
def extract_process_vec(file_path,tfidf, w2v, c2v):
    ...
    while True:
        line = f.readline()
        if line == '\n':
            process_vec[pname].append(np.nanmean(tmp_process_vec,axis=0).tolist())
            id += 1
            tmp_process_vec = []
            isprocess_file = True
            continue
        ...
        if isinstance(r,float):
            continue
        #print(filepath+str(r)+str(len(r.tolist())))
...
if __name__ == "__main__":
    ...
    file_vec, fileid2name, file_freq, process_num = extract_process_feature(inputfile,w2v,c2v)
    threshold = 0.9
    ...
    for i in range(cos.shape[0]):
        ...
        process_set = set()
        for j in index:
            process_set |= file_freq[fileid2name[j]]#|=为并集
