...
def extract_process_feature(file_path,tfidf, stability, w2v, c2v):
    ...
    f = open(file_path,'r')
    print('start graph')
    process_vec = defaultdict(list)#进程的向量表示
    id = 0
...
if __name__ == "__main__":
    ...
    data_len = len(list(process_vec.keys()))# 计算良性数据中进程的总数
    print(data_len)
    train_data = defaultdict(list)#训练数据
    train_data2 = defaultdict(list)#验证数据
    per = data_len
    cnt = 0
    keys = list(process_vec.keys())#所有进程名
    random.shuffle(keys)#打乱训练数据

    train_data = process_vec
        
    print('train:',len(list(train_data.keys())))
    out_embedd1 = 'process_embedding_train.json'#训练数据的输出
...
#加载训练数据
    ...
    for epoch in range(epochs):
        ...
        for bx, (data) in enumerate(train_):
            data = data.to(device)
            sample = model(data)#重构样本
            loss = criterion(data.to(device), sample) + model.encoder.kl#重构差异，两者之间的MSELoss值
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
# 初始化三个空列表，用于存储标签、进程名称和损失值
    ...
    for i in valid_data:
        try:
            ...
            name = 'None' 
        data = torch.FloatTensor(valid_data[i])# 将嵌入表示转换为FloatTensor
        sample = model(data.to(device))# 使用模型对数据进行重构
        loss = criterion(data.to(device),sample).item()# 计算原始数据和重构数据之间的损失
        if name in stability:
            loss = loss / (math.log(stability[name]) + 1)#使用稳定性权重调整损失值
        loss_dist.append(loss)



    anomaly_std = np.std(np.array(loss_dist))# 计算损失分布的标准差
    anomaly_mean = np.mean(np.array(loss_dist))# 计算损失分布的均值
    # cov = grubbs.max_test_outliers(loss_dist, alpha=0.05)

    anomaly_cutoff = np.percentile(np.array(loss_dist),90)#计算阈值
